...
import { createChat, delAllChats, delChat, getAllChats } from "../../api/chats/chats";
...
const actions = {
  ...
  async addChat({commit}: any,payload: {theme_id: number, user_id: number,question: string}) {
    createChat({'theme_id':payload.theme_id, 'user_id': payload.user_id})
      .then((data: Omit<Chat,'messages','is_saved'>) => {
