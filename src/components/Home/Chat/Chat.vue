...
import { ref, onMounted, nextTick, PropType, computed } from "vue";
import ChatItem from "../ChatItem/ChatItem.vue";
import { createMessage, SelectedChatState } from "../../../store/modules/selectedChat";
import { client } from "../../../api/fetchClient";
import { AiResponseMessage, Message, SavedMessage } from "../../../store/types/MessagesType";
...
const handleSubmit = () => {
  if (!props.selectedMessage) {
    ...
    chatsItems.value.push({'content': text.value,'is_ai_response': false, 'id': lastIndex + 1})

    client.post(`/chats/${props.guestChatId}/guest/message`,{'content': text.value})
      .then((data:{ response: string}) => {
        
        chatsItems.value.push({'content': data.response,'is_ai_response': true, 'id': lastIndex + 1})
      })
    } else if (!props.guestChat && text.value.trim() !== '' && props.selectedChat?.selectedChat === null) {
      const obj = {
      ...
      client.post('/chats/create',obj).then((data: Chat) => {
        ...
        client.post(`/chats/${data.id}/message`,{'content': text.value}).then((data: AiResponseMessage) => {
          const chatData = {
    ...
    } else if (!props.guestChat && text.value.trim() !== '' && props.selectedChat?.selectedChat) {
  ...
  <div class="chat">
    <div class="chat__header">
      <button class="chat__header-button" @click="createNewChat">
        <svg  viewBox="0 0 23 23" fill="none" xmlns="http://www.w3.org/2000/svg" class="chat__header-button--svg">
          <path
      ...
      <div class="chat__title"> Health</div>
    ...
    <div class="chat__main">
      ...
      />
      <ChatItem
        v-if="!guestChat && selectedChat === null"
