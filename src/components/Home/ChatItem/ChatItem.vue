...
import { saveMessage, unsaveMessage } from '../../../store/modules/savedMessages';
...
console.log(!props.item?.is_ai_response)
</script>

<template>
  <div
    class="chat-item"
    :class="{ 'chat-item--ai': item?.is_ai_response, 'chat-item--user': !item?.is_ai_response}"
  >
    ...
    >
      <div 
        ...
        v-html="isLoading && isAiTyping ? marked(typedContent) : marked(item?.content)"
      >
      </div>
      <div 
        v-if="savedMessage"
